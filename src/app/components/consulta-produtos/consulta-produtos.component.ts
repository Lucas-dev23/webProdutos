...
export class ConsultaProdutosComponent implements OnInit {
  ...
  produtos: any[] = [];
  paginador: number = 1;
  produto: any = {}; //capturando o produto que foi selecionado para exibir na modal
  mensagem: string = '';
  ...
  //função para capturar os dados do produto que será exibido na modal
    ...
    this.httpClient.delete(`http://localhost:8081/api/produtos/${this.produto.id}`)
      .subscribe({
        next: (data: any) => {
          ...
          this.ngOnInit(); //executando a consulta novamente para que o produto suma da tela ao ser excluído
        },
        error: (e) => {
          console.log(e.error);
        }
